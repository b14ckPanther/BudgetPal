/**
 * BudgetPal — New Transaction Screen
 * Manual transaction entry with full validation and dynamic categories.
 */
...
import { Calendar, ChevronDown, PenLine, ArrowLeft } from 'lucide-react-native';
...
export default function NewTransactionScreen() {
  ...
  const filteredCategories = useMemo(() => {
    ...
    // Get parent categories of correct type
    ...
    // Transfer matches transfer or savings or fallback to other
    if (type === 'transfer') {
      const matches = parents.filter((c) => c.type === 'transfer' || c.type === 'savings');
      if (matches.length > 0) return matches;
      // Fallback if no specific transfer categories exist
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const savedMerchant = params.merchant?.trim() ? label : (params.title ? undefined : label);
    ...
    <Screen>
      ...
      >
        ...
        >
          ...
          {/* Subcategory Dropdown (only if parent category has subcategories) */}

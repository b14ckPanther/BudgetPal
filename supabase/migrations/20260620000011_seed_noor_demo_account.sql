-- Presentation demo account: shared seed/clear functions for Noor (no auth secrets).
-- Auth user is provisioned via scripts/demo-provision-noor.mjs (Supabase Admin API).

-- Stable demo user UUID ("noor" = 6e6f6f72 in hex prefix).
CREATE OR REPLACE FUNCTION public.demo_noor_user_id()
RETURNS uuid
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT '6e6f6f72-6465-4000-8000-000000000001'::uuid;
$$;

CREATE OR REPLACE FUNCTION public.is_demo_noor_user(p_user_id uuid)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT p_user_id = public.demo_noor_user_id();
$$;

CREATE OR REPLACE FUNCTION public.demo_category_id(
  p_user_id uuid,
  p_name text,
...
AS $$
  ...
  WHERE c.user_id = p_user_id
    AND c.name = p_name
    AND (
      (p_parent_id IS NULL AND c.parent_category_id IS NULL)
      OR c.parent_category_id = p_parent_id
    )
  LIMIT 1;
$$;

CREATE OR REPLACE FUNCTION public.assert_demo_noor_user(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NOT public.is_demo_noor_user(p_user_id) THEN
    RAISE EXCEPTION 'Refusing demo seed/clear for non-demo user %', p_user_id;
  END IF;
END;
$$;

-- Clears mutable presentation data for the demo user only (not profile/categories/budget row).
CREATE OR REPLACE FUNCTION public.clear_demo_noor_mutable_data(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  DELETE FROM public.agent_messages WHERE user_id = p_user_id;
  DELETE FROM public.transactions WHERE user_id = p_user_id;
  DELETE FROM public.receipts WHERE user_id = p_user_id;
  DELETE FROM public.voice_entries WHERE user_id = p_user_id;
  DELETE FROM public.reports WHERE user_id = p_user_id;
  DELETE FROM public.warnings WHERE user_id = p_user_id;
  DELETE FROM public.budget_events WHERE user_id = p_user_id;
  DELETE FROM public.budget_category_limits WHERE user_id = p_user_id;
END;
$$;
...
BEGIN
  ...
  IF (SELECT count(*) FROM public.categories WHERE user_id = p_user_id) < 10 THEN
  ...
  UPDATE public.budgets SET is_active = false
  ...
  INSERT INTO public.transactions (
    ...
    (p_user_id, 2500, 'ILS', 'income', 'Freelance client', 'Design project', v_income, v_side, v_cycle_start + 11, 'manual', 'confirmed');

  -- Current cycle: Food & Drinks ~81% of 2200 (near limit)
  INSERT INTO public.transactions (
    user_id, amount, currency, type, merchant, title, category_id, subcategory_id, date, source, status
  ) VALUES
    (p_user_id, 318, 'ILS', 'expense', 'Shufersal', 'Weekly groceries', v_food, v_groceries, v_cycle_start + 2, 'manual', 'confirmed'),
    (p_user_id, 276, 'ILS', 'expense', 'Rami Levy', 'Groceries', v_food, v_groceries, v_cycle_start + 8, 'manual', 'confirmed'),
    (p_user_id, 24, 'ILS', 'expense', 'Aroma', 'Morning coffee', v_food, v_coffee, v_cycle_start + 3, 'text', 'confirmed'),
    (p_user_id, 22, 'ILS', 'expense', 'Cafeneto', 'Coffee', v_food, v_coffee, v_cycle_start + 6, 'voice', 'confirmed'),
    (p_user_id, 94, 'ILS', 'expense', 'Wolt', 'Dinner delivery', v_food, v_delivery, v_cycle_start + 5, 'manual', 'confirmed'),
    (p_user_id, 86, 'ILS', 'expense', 'Wolt', 'Lunch delivery', v_food, v_delivery, v_cycle_start + 12, 'manual', 'confirmed'),
    (p_user_id, 62, 'ILS', 'expense', 'McDonald''s', 'Quick lunch', v_food, v_restaurants, v_cycle_start + 4, 'manual', 'confirmed'),
    (p_user_id, 118, 'ILS', 'expense', 'Miznon', 'Dinner out', v_food, v_restaurants, v_cycle_start + 9, 'manual', 'confirmed'),
    (p_user_id, 204, 'ILS', 'expense', 'Tiv Taam', 'Groceries', v_food, v_groceries, v_cycle_start + 14, 'manual', 'confirmed'),
    (p_user_id, 198, 'ILS', 'expense', 'Super Yuda', 'Groceries', v_food, v_groceries, v_cycle_start + 17, 'manual', 'confirmed'),
    (p_user_id, 21, 'ILS', 'expense', 'Aroma', 'Coffee', v_food, v_coffee, v_cycle_start + 15, 'text', 'confirmed'),
    (p_user_id, 74, 'ILS', 'expense', 'Landwer', 'Breakfast', v_food, v_restaurants, v_cycle_start + 18, 'manual', 'confirmed'),
    (p_user_id, 167, 'ILS', 'expense', 'Shufersal', 'Groceries', v_food, v_groceries, current_date - 1, 'manual', 'confirmed');

  -- Car ~105% of 1200 (slightly over)
  INSERT INTO public.transactions (
    user_id, amount, currency, type, merchant, title, category_id, subcategory_id, date, source, status
  ) VALUES
    (p_user_id, 285, 'ILS', 'expense', 'Paz', 'Fuel', v_car, v_car_fuel, v_cycle_start + 1, 'manual', 'confirmed'),
    (p_user_id, 268, 'ILS', 'expense', 'Sonol', 'Fuel', v_car, v_car_fuel, v_cycle_start + 10, 'manual', 'confirmed'),
    (p_user_id, 312, 'ILS', 'expense', 'Delek', 'Fuel', v_car, v_car_fuel, v_cycle_start + 16, 'manual', 'confirmed'),
    (p_user_id, 28, 'ILS', 'expense', 'Pango', 'Parking', v_car, v_car_parking, v_cycle_start + 7, 'manual', 'confirmed'),
    (p_user_id, 32, 'ILS', 'expense', 'Pango', 'Parking', v_car, v_car_parking, v_cycle_start + 13, 'manual', 'confirmed'),
    (p_user_id, 348, 'ILS', 'expense', 'Auto Care', 'Oil change', v_car, v_car_maint, v_cycle_start + 6, 'manual', 'confirmed');

  -- Shopping healthy ~37%
  INSERT INTO public.transactions (
    user_id, amount, currency, type, merchant, title, category_id, date, source, status
  ) VALUES
    (p_user_id, 129, 'ILS', 'expense', 'H&M', 'Clothing', v_shopping, v_cycle_start + 4, 'manual', 'confirmed'),
    (p_user_id, 89, 'ILS', 'expense', 'KSP', 'Phone accessory', v_shopping, v_cycle_start + 11, 'manual', 'confirmed'),
    (p_user_id, 80, 'ILS', 'expense', 'Zara', 'Shirt', v_shopping, v_cycle_start + 19, 'manual', 'confirmed');

  -- Bills predictable
  INSERT INTO public.transactions (
    user_id, amount, currency, type, merchant, title, category_id, subcategory_id, date, source, status
  ) VALUES
    (p_user_id, 3500, 'ILS', 'expense', 'Landlord', 'Rent', v_bills, v_rent, v_cycle_start + 1, 'manual', 'confirmed'),
    (p_user_id, 119, 'ILS', 'expense', 'Bezeq', 'Internet', v_bills, v_internet, v_cycle_start + 2, 'manual', 'confirmed'),
    (p_user_id, 89, 'ILS', 'expense', 'Cellcom', 'Mobile plan', v_bills, v_phone, v_cycle_start + 2, 'manual', 'confirmed'),
    (p_user_id, 198, 'ILS', 'expense', 'IEC', 'Electricity', v_bills, public.demo_category_id(p_user_id, 'Electricity', v_bills), v_cycle_start + 5, 'manual', 'confirmed');

  -- Subscriptions recurring signals
  INSERT INTO public.transactions (
    user_id, amount, currency, type, merchant, title, category_id, subcategory_id, date, source, status
  ) VALUES
    (p_user_id, 49.90, 'ILS', 'expense', 'Netflix', 'Streaming', v_subs, v_streaming, v_cycle_start + 3, 'manual', 'confirmed'),
    (p_user_id, 19.90, 'ILS', 'expense', 'Spotify', 'Music', v_subs, v_streaming, v_cycle_start + 3, 'manual', 'confirmed'),
    (p_user_id, 32, 'ILS', 'expense', 'Apple', 'iCloud', v_subs, public.demo_category_id(p_user_id, 'Apps', v_subs), v_cycle_start + 4, 'manual', 'confirmed');

  -- Entertainment, Health, Transport
  INSERT INTO public.transactions (
    user_id, amount, currency, type, merchant, title, category_id, date, source, status
  ) VALUES
    (p_user_id, 156, 'ILS', 'expense', 'Cinema City', 'Movies', v_ent, v_cycle_start + 8, 'manual', 'confirmed'),
    (p_user_id, 124, 'ILS', 'expense', 'Ticketmaster', 'Concert tickets', v_ent, v_cycle_start + 15, 'manual', 'confirmed'),
    (p_user_id, 145, 'ILS', 'expense', 'Super-Pharm', 'Pharmacy', v_health, v_cycle_start + 7, 'manual', 'confirmed'),
    (p_user_id, 86, 'ILS', 'expense', 'Gett', 'Taxi', v_transport, v_cycle_start + 5, 'manual', 'confirmed'),
    (p_user_id, 62, 'ILS', 'expense', 'Rav-Kav', 'Transit pass', v_transport, v_cycle_start + 1, 'manual', 'confirmed'),
    (p_user_id, 97, 'ILS', 'expense', 'Gett', 'Taxi', v_transport, v_cycle_start + 14, 'manual', 'confirmed');

  -- Receipt + voice showcase (current cycle)
  ...
  WHERE t.user_id = p_user_id
    ...
    AND t.amount = 22
    AND t.source = 'voice'
    AND t.date = v_cycle_start + 6;

  -- One old rejected expense for lifecycle coverage
  INSERT INTO public.transactions (
    user_id, amount, currency, type, merchant, title, category_id, date, source, status
  ) VALUES (
...
REVOKE ALL ON FUNCTION public.clear_demo_noor_mutable_data(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.seed_demo_noor_baseline(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.clear_demo_noor_mutable_data(uuid) TO service_role;
